...
LINUX_URL="http://11c8g.seq.ink:9999/deepin23.zip"
echo "下载系统文件中..."

Linux_zip_MD5="8f20cc201b1a84be3a61c2219dcd3f1b"

wget -c $LINUX_URL -O linux.zip
CALCULATED_L_MD5=$(md5sum "linux.zip" | awk '{ print $1 }')
if [ "$Linux_zip_MD5" != "$CALCULATED_L_MD5" ]; then
    echo "系统镜像校验失败！请尝试重新运行脚本下载，若依然失败请手动删除当前目录下的linux.zip文件重试。"
    exit 1
else
    echo "文件校验成功！"
fi
